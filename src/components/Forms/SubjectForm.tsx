    ...
    <form className="flex flex-col gap-8" onSubmit={onSubmit}>
      ...
      <div className="flex justify-between items-center">
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          {errors.sex?.message && <p className="w-full text-xs text-red-400">{errors.sex.message.toString()}</p>}
        </div>
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          <label className="text-xs text-gray-500 flex items-center gap-2 cursor-pointer" htmlFor="image">
            <Image src="/upload.png" alt="Upload" width={28} height={28} />
            <span>Upload a photo</span>
          </label>
          <input type="file" id="image" {...register("image")} className="hidden" />
          {errors.image?.message && <p className="w-full text-xs text-red-400">{errors.image.message.toString()}</p>}
        </div>
      </div>
